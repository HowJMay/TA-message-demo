...
retcode_t mam_example_announce_channel(mam_api_t* const api,
  ...
  ERR_BIND_RETURN(
      mam_api_channel_create(api, MAM_EXAMPLES_MSS_HEIGHT, new_channel_id),
      ret);

  if ((ret = mam_api_bundle_announce_channel(api, channel_id, new_channel_id,
                                             psks, NULL, bundle, msg_id)) !=
      RC_OK) {
    return ret;
  }
...
retcode_t mam_example_announce_endpoint(mam_api_t* const api,
  ...
  ERR_BIND_RETURN(mam_api_endpoint_create(api, MAM_EXAMPLES_MSS_HEIGHT,
                                          channel_id, new_endpoint_id),
                  ret);

  if ((ret = mam_api_bundle_announce_endpoint(api, channel_id, new_endpoint_id,
                                              psks, NULL, bundle, msg_id)) !=
      RC_OK) {
    return ret;
  }
